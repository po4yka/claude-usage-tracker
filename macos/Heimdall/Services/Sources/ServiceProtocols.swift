...
public protocol SettingsStore: Sendable {
    func load() -> HeimdallConfig
    func save(_ config: HeimdallConfig) throws
...
public protocol LocalNotificationCoordinating: Sendable {
    func handleConfigChange(previous: HeimdallConfig, current: HeimdallConfig) async -> AppIssue?
    func process(envelope: ProviderSnapshotEnvelope, config: HeimdallConfig) async -> AppIssue?
}
...
public protocol StartupOptimizedProviderDataSource: ProviderDataSource {
    func fetchStartupSnapshots(config: HeimdallConfig) async throws -> ProviderSnapshotEnvelope
}
