...
public actor LocalNotificationCoordinator: LocalNotificationCoordinating {
    ...
    public func handleConfigChange(previous: HeimdallConfig, current: HeimdallConfig) async -> AppIssue? {
    ...
    public func process(envelope: ProviderSnapshotEnvelope, config: HeimdallConfig) async -> AppIssue? {
    ...
    private static func permissionDeniedIssue() -> AppIssue {
        AppIssue(
            ...
            message: "Local notifications are enabled, but macOS notification permission is denied. Enable Heimdall in System Settings > Notifications."
        )
    }
...
public struct NoopLocalNotificationCoordinator: LocalNotificationCoordinating {
    ...
    public func handleConfigChange(previous _: HeimdallConfig, current _: HeimdallConfig) async -> AppIssue? {
        nil
    }

    public func process(envelope _: ProviderSnapshotEnvelope, config _: HeimdallConfig) async -> AppIssue? {
        nil
    }
}
