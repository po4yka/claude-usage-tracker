...
public struct TerminalAuthCommandRunner: AuthCommandRunning {
    ...
    public func runAuthCommand(
        ...
        let scriptURL = self.temporaryDirectoryProvider()
            .appendingPathComponent("heimdall-\(provider.rawValue)-auth.command", isDirectory: false)
    ...
    static func scriptContents(
        ...
        clear
        echo "Heimdall \(provider.title) Auth Recovery"
        echo
        if [ -z "$CLI_PATH" ] || [ ! -x "$CLI_PATH" ]; then
          echo "\(provider.title) CLI executable could not be resolved."
          echo "Heimdall checked both Terminal's PATH and your login-shell PATH."
        ...
        if [ "\(provider.rawValue)" = "claude" ]; then
          if [ "$(uname)" = "Darwin" ]; then
            ...
            echo "If login completed, refresh Heimdall even when ~/.claude/.credentials.json is absent."
        ...
        echo "Return to Heimdall and refresh \(provider.title)."
        echo
        read -k '?Press any key to close...'
        """
    }
