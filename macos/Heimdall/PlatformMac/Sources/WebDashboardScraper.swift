...
public final class WebDashboardScraper {
    ...
    public func fetch(
        ...
        if !allowLiveNavigation {
            return ScrapeResult(
                ...
                detailLines: ["Live web scraping runs only inside Heimdall.app.", "Open the app or refresh the menu bar to populate cached web dashboard extras for CLI reads."],
                webExtras: nil,
                isLoginRequired: false,
                fetchedAt: nowStamp
            )
    ...
    private static func cacheURL(for provider: ProviderID) -> URL? {
        ...
        return root
            .appendingPathComponent("dev.po4yka.heimdall", isDirectory: true)
