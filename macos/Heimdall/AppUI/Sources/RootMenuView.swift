...
enum WindowReopener {
    ...
    private static func hasExistingMainWindow() -> Bool {
        NSApplication.shared.windows.contains { window in
            window.identifier?.rawValue == HeimdallSceneID.mainWindow
        }
    }

    @MainActor
    private static func focusExistingMainWindow() {
        guard let window = NSApplication.shared.windows.first(where: { $0.identifier?.rawValue == HeimdallSceneID.mainWindow }) else {
            return
        }
    ...
    static func reopenMainWindow(
        ...
        openWindow(HeimdallSceneID.mainWindow)
    }
}
