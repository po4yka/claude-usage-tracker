            ...
            if webSessionReady {
                return ProviderSourceResolution(
                    ...
                    explanation: webUsageAvailable
                        ? "No live helper snapshot is available, so Heimdall is using the cached web dashboard quotas."
        ...
        case .oauth, .cli:
            ...
            if liveSource == requestedSource, snapshot.available {
                ...
                if snapshot.sourceUsed == "admin" {
                    warnings.append("OAuth is unavailable, so Heimdall is using Anthropic admin analytics fallback.")
                }
                return ProviderSourceResolution(
                    provider: provider,
                    requestedSource: requestedSource,
