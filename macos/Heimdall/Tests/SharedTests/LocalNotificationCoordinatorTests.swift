...
private actor LocalNotificationCoordinatorSpy: LocalNotificationCoordinating {
    ...
    func handleConfigChange(previous _: HeimdallConfig, current _: HeimdallConfig) async -> AppIssue? {
        self.configChangeCalls += 1
        return nil
    }

    func process(envelope _: ProviderSnapshotEnvelope, config _: HeimdallConfig) async -> AppIssue? {
        self.processCalls += 1
        return nil
    }
...
private final class SettingsStoreSpy: @unchecked Sendable, SettingsStore {
    private(set) var savedConfigs: [HeimdallConfig] = []

    func load() -> HeimdallConfig { .default }
