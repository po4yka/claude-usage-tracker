...
public enum CLIArgumentParser {
    public static func parse(arguments: [String]) throws -> CLIInvocation {
        ...
        case "config":
            ...
            default:
                throw CLIArgumentError.invalidArguments("config expects validate|dump; run `heimdall config --help`")
            }
        case "auth":
            let rest = Array(args.dropFirst())
            if rest.first.map(Self.isHelpFlag) != false {
            ...
            default:
                throw CLIArgumentError.invalidArguments("auth expects status|doctor|login; run `heimdall auth --help`")
            }
        default:
            throw CLIArgumentError.invalidArguments("unknown command: \(first); run `heimdall --help`")
        }
    }
    ...
    public static func helpText(for topic: CLIHelpTopic) -> String {
        ...
        case .root:
            return """
            Usage:
              heimdall usage [options]
              heimdall cost [options]
              heimdall auth status [options]
              heimdall auth doctor [options]
              heimdall auth login --provider claude|codex [--device]
              heimdall config validate [--format text|json] [--pretty]
              heimdall config dump [--format json] [--pretty]
            ...
            Commands:
              ...
              config  Validate or dump Heimdall configuration.
        ...
        case .config:
            ...
            Subcommands:
              validate   Validate the stored Heimdall configuration.
              dump       Emit the current Heimdall configuration as JSON.
            """
        case .auth:
            return """
            Usage:
