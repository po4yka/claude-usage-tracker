...
`HeimdallBar` is the native macOS companion app for Claude Code and Codex. It is different from the existing `statusline` and `menubar` surfaces:
...
- `HeimdallBar` is a standalone `LSUIElement` macOS app with native menu bar UI, widgets, browser-session import, optional web extras, and the bundled `heimdallbar` parity CLI.

Install the prebuilt macOS app artifact from the [GitHub Releases](https://github.com/po4yka/heimdall/releases) page:

```bash
VERSION=$(curl -fsSL https://api.github.com/repos/po4yka/heimdall/releases/latest | jq -r '.tag_name')
curl -fL -o "/tmp/heimdallbar-${VERSION}.zip" \
  "https://github.com/po4yka/heimdall/releases/download/${VERSION}/heimdallbar-${VERSION}-macos-app.zip"
unzip -q "/tmp/heimdallbar-${VERSION}.zip" -d /tmp
open "/tmp/heimdallbar-${VERSION}-macos-app/HeimdallBar.app"
```

Recommended post-install layout:

```bash
mv "/tmp/heimdallbar-${VERSION}-macos-app/HeimdallBar.app" /Applications/
open /Applications/HeimdallBar.app
```

#### What launches automatically

When `HeimdallBar.app` starts, it manages its own local helper:
...
```bash
export DYLD_FRAMEWORK_PATH="/tmp/heimdallbar-${VERSION}-macos-app/Frameworks"
"/tmp/heimdallbar-${VERSION}-macos-app/bin/heimdallbar" config dump
"/tmp/heimdallbar-${VERSION}-macos-app/bin/heimdallbar" usage --provider both --format json --pretty
```

#### Widgets

`HeimdallBar` ships four widget families for Claude Code and Codex:
...
- the app stores session metadata such as provider, browser source, import time, and cookie-domain presence for state display
- hidden WebKit scraping is opt-in and only used for provider-specific dashboard extras
- helper traffic stays on loopback
- HeimdallBar does not require a remote Heimdall service

Practical implication: if you never enable web extras or import a browser session, HeimdallBar still works from Rust-backed local and OAuth-derived data only.

#### Troubleshooting

Helper not reachable:

- relaunch `HeimdallBar.app`; the app should restart its bundled helper automatically
...
- launch `HeimdallBar.app` at least once before opening the widget gallery
...
- `Contents/PlugIns/HeimdallBarWidget.appex`
